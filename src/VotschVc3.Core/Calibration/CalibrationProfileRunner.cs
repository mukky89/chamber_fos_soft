...
public sealed class CalibrationProfileRunner
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        List<ExecutionStep> calibrationSteps = steps.Where(s => !s.Segment.IsRamp && s.Segment.IsCalibrationPoint).ToList();
        ...
        try
        {
            foreach (ExecutionStep step in steps)
            {
                ...
                if (step.Segment.IsRamp || !step.Segment.IsCalibrationPoint)
                {
                    continue;
                }
