...
public sealed class CalibrationDashboardViewModel : INotifyPropertyChanged
{
    ...
    private void RefreshSteps()
    {
        ...
        string[] tips =
        {
            ...
            $"Každý vybraný peak má vlastný detektor. Potrebuje {_requiredStableSamples} vzoriek, range ≤ {_maxRangePm:F3} pm, σ ≤ {_maxStdDevPm:F3} pm a drift ≤ {_maxPeakDriftPmPerMinute:F3} pm/min. Peaky sa kontrolujú paralelne; {Estimate(_requiredStableSamples)}. Základný timeout peaku: {Duration(_sensorTimeout)}. Po prvom zrušení rozpracovaného finálneho merania sa k nemu pripočíta čas na jeden celý nový pokus vypočítaný zo skutočného cyklu dát. Ďalšie restarty už limit nepredlžujú. {cycle}.",
