...
public sealed class SylexFosCalibrationIntegration : IAsyncDisposable
{
    ...
    public SylexFosCalibrationIntegration(CalibrationViewModel viewModel)
    {
        ...
        string baseUrl = Environment.GetEnvironmentVariable("SYLEX_FOS_API_URL")
            ?? SylexFosApiSettings.DefaultBaseUrl;
    ...
    private void OnPeaksChanged(object? sender, NotifyCollectionChangedEventArgs e)
    {
        ...
        {
            foreach (CalibrationPeakRowViewModel row in _lookups.Keys.ToArray())
                DetachRow(row);
            foreach (CalibrationPeakRowViewModel row in _viewModel.Peaks)
                AttachRow(row);
        }
    ...
    {
        if (sender is not CalibrationPeakRowViewModel row)
            return;

        if (e.PropertyName == nameof(CalibrationPeakRowViewModel.SerialNumber))
            ScheduleLookup(row);
    }
    ...
    private async Task LookupAndApplyAsync(
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            // Barcode scanners and manual entry may raise multiple quick property changes.
            await Task.Delay(250, cancellationToken).ConfigureAwait(false);
            ProductionMetadata? metadata = await _metadataProvider
                .FindAsync(serialNumber, row.Channel, cancellationToken)
                .ConfigureAwait(false);

            if (metadata is null || cancellationToken.IsCancellationRequested)
                return;

            await Application.Current.Dispatcher.InvokeAsync(() =>
            {
                // Do not apply stale metadata when the operator changed the SN during the request.
                if (!string.Equals(row.SerialNumber, serialNumber, StringComparison.OrdinalIgnoreCase))
                    return;

                if (!string.IsNullOrWhiteSpace(metadata.ProductDescription))
                    row.ProductDescription = metadata.ProductDescription;
                if (!string.IsNullOrWhiteSpace(metadata.Customer))
                    row.Customer = metadata.Customer;
                if (!string.IsNullOrWhiteSpace(metadata.Order))
                    row.Order = metadata.Order;
            });

            AppLog.Info(
                "Sylex FOS API",
                $"FBG SN {serialNumber}: produkčné metadata načítané pre kalibráciu.");
        }
        catch (OperationCanceledException)
        {
            // Normal debounce / row replacement.
        }
        ...
        {
            // API enrichment must never prevent the operator from continuing calibration.
            AppLog.Warn(
                "Sylex FOS API",
                $"FBG SN {serialNumber}: metadata sa nepodarilo načítať ({ex.Message}). Polia zostávajú editovateľné.");
        }
        finally
        {
            if (_lookups.TryGetValue(row, out CancellationTokenSource? current) &&
    ...
    private async Task CheckApiAsync()
    {
        ...
        {
            AppLog.Info("Sylex FOS API", $"Centrálne API je dostupné na {_apiClientBaseUrl()}.");
        }
        else
        {
            AppLog.Warn(
                "Sylex FOS API",
                $"Centrálne API nie je dostupné ({health.Status}). Kalibrácia môže pokračovať bez automatického doplnenia metadata.");
        }
    }

    private static string _apiClientBaseUrl() =>
        Environment.GetEnvironmentVariable("SYLEX_FOS_API_URL")
        ?? SylexFosApiSettings.DefaultBaseUrl;
