...
- The compact WIKA card uses one framed dual-axis chart with a shared time axis: temperature in °C on the left and accumulated stable time in seconds on the right. Keep the two scales visually and numerically distinct.
...
- Peak identity uncertainty is a hard exclusion, separate from wavelength instability. `SkippedIdentityUncertain` automatically skips affected channel targets without operator prompts even with supervision enabled. Never force-sample or fit these targets. Preserve original detections and assignment audit; a merge, count change, ambiguous matching or observation gap latches the channel for the run, including checkpoint resume. Unaffected targets continue. Ordinary skipped plateaus advance immediately when all targets are terminal, but the final 25 °C return must still satisfy its temperature gate before normal completion STOP.
...
- New calibration setups default to WIKA target tolerance 1 °C, stable duration 10 minutes, setpoint ramp 1 °C/min, maximum WIKA drift 0.03 °C/min, FBG acquisition interval 30 seconds, 50 stability samples and 50 final samples. Saved setups retain their other settings, but every new run takes the saved Admin sample-acquisition interval at setup load and again at start. Active runs and checkpoints retain their interval snapshot until the operator explicitly applies current defaults. The setpoint ramp speed remains editable during an active run and is applied from the next ramp command step without resetting WIKA or FBG windows; the ramp enable switch remains locked while running.
...
- Stopping an active FBG calibration or exiting the application during one must ask whether to send STOP or leave the chamber regulating its current setpoint. Keeping the chamber running still saves the checkpoint, cancels measurement, disconnects cleanly and must suppress every cancellation-path STOP; safety trips and an explicit STOP remain authoritative.
...
- After the bounded FBG wavelength-stability budget (base with 20% reserve, progress extensions up to 90 minutes), always collect a separate bounded final sample window even if wavelength stability failed. This applies with operator supervision too. Mark measured results CompletedWithStabilityWarning and include finite measured points in coefficient analysis with explicit problems; do not pretend missing measurements or singular models have coefficients.
- Final flagged sampling uses the configured final sample count/cadence, at most 90 additional minutes, and does not reset on wavelength instability. WIKA remains authoritative. Preserve valid partial samples with explicit incompleteness; missing data produces an N/A evaluation row.
...
- After planned calibration plateaus, always return to 25 °C, perform configured conditioning, then an independent final verification sample window. Use the released coefficients to predict wavelength at measured WIKA temperature and report wavelength/temperature residuals; never add verification samples to the fit. STOP/cancellation and hardware safety remain authoritative.
