...
- Under `ContinueAndFlag`, exhausting the dynamic FBG deadline produces a TimedOut result and continues without extra fallback sampling. Keep acquired data for diagnostics; never accept incomplete/unstable results as valid calibration. The base covers stability plus final samples with 20% reserve; progress may extend it in 10-minute steps, always capped at 90 minutes from the first FBG phase start, including reference loss and resets.
- Historical three-acquisition fallback results remain readable, but new timed-out peaks must not start fallback acquisition.
