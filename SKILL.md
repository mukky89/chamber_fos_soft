...
- New calibration setups default to WIKA target tolerance 1 °C, stable duration 10 minutes, setpoint ramp 2 °C/min, maximum WIKA drift 0.03 °C/min, FBG acquisition interval 30 seconds, 50 stability samples and 50 final samples. Saved setups retain their other settings, but every new run takes the saved Admin sample-acquisition interval at setup load and again at start. Active runs and checkpoints retain their interval snapshot until the operator explicitly applies current defaults.
